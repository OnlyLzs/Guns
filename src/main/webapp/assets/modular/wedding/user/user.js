layui.use(['layer', 'form', 'table', 'ztree', 'laydate', 'admin', 'ax'], function () {
    ...
    MgrUser.initColumn = function () {
        return [[
            ...
            {field: 'login_name', sort: true, title: '登录账号'},
    ...
    MgrUser.onDeleteUser = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/wedding/app_user/delete", function () {
    ...
    MgrUser.resetPassword = function (data) {
        Feng.confirm("是否重置密码为111111 ?", function () {
            var ajax = new $ax(Feng.ctxPath + "/wedding/app_user/reset", function (data) {
                Feng.success("重置密码成功!");
            }, function (data) {
                Feng.error("重置密码失败!");
            });
    ...
    MgrUser.changeUserStatus = function (userId, checked) {
        if (checked) {
            var ajax = new $ax(Feng.ctxPath + "/wedding/app_user/status", function (data) {
                Feng.success("解除冻结成功!");
            }, function (data) {
                Feng.error("解除冻结失败!");
                table.reload(MgrUser.tableId);
    ...
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/wedding/app_user/list',
        page: true,
        height: "full-158",
        cellMinWidth: 100,
